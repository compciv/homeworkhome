...
def wrangle_inmate_data_from_tag(rowtag):
    ...
    idict = {}
    idict['tdcj_id'] = cols[0].text.strip()
    href = cols[1].select('a')[0].attrs['href'].strip()
    idict['url'] = make_absolute_url(href).strip()
    idict['last_name'] = cols[2].text.strip()
    idict['first_name'] = cols[3].text.strip()
    idict['birthdate'] = txdate_to_iso(cols[4].text.strip())
    idict['gender'] = cols[5].text.strip()
    idict['race'] = cols[6].text.strip()
    idict['date_received'] = txdate_to_iso(cols[7].text.strip())
    idict['county'] = cols[8].text.strip()
    idict['date_offense'] = txdate_to_iso(cols[9].text.strip())
    idict['age_at_offense'] = round(calc_years_diff(idict['birthdate'].strip(), idict['date_offense'].strip()))
    idict['years_before_death_row'] = calc_years_diff(idict['date_offense'].strip(), idict['date_received'].strip())
