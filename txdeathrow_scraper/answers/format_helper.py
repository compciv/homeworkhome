...
def txdate_to_iso(datestr):
    ...
    # if the size/length of the year part is NOT 4 characters,
    # e.g. '88' instead of '1988'
    if len(y) != 4:
        # assume it is a year in the 20th century, i.e. 2088
        y = '19' + y
    return '-'.join([y, m, d])
...
def calc_years_diff(start_date, end_date):
    ...
    Recommendation:
        ...
        Convert the strings into datetime.datetime objects. Then subtract the
          ...
          of a difference...
    """
    dx = datetime.strptime(start_date, '%Y-%m-%d')
    dy = datetime.strptime(end_date, '%Y-%m-%d')
    diff = dy - dx
    return round(diff.days / 365.0, 1)
...
def make_absolute_url(href):
    ...
    Returns:
      <str>
    """
    abs_url = data_helper.DATA_SRC_URL
